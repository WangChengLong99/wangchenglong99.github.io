...
```{r Null,echo=FALSE}
knitr::include_graphics("static/image/数据/缺失值处理.png")
```
...
knitr::include_graphics("static/image/数据/箱线图.png")
```
...
```{r decide-abnormal,echo=FALSE}
knitr::include_graphics("static/image/数据/判断异常.png")
```

```{python plot-box-abnormal}
...
knitr::include_graphics("static/image/数据/正态分布.png")
```

可以按照下表中的判断条件，识别出数值型变量的异常点和极端异常点，如下表所示：

```{r abnormal-point,echo=FALSE}
knitr::include_graphics("static/image/数据/异常点.png")
```

```{python plotab,fig.cap="异常数据的折线图"}
