...
```{python select-one-column,results="asis"}
(
pd.DataFrame(
  [
...
```{python select-many-column,results="asis"}
(
  pd.DataFrame(
    [
...
```{python select-column-dtype,results="asis"}
(
...
```{python add-column,warning=FALSE,results="asis"}
# 增加相同长度的序列
...
```{python add-many-column,warning=FALSE,results="asis"}
# 直接赋值
...
```{python insert-column,results="asis"}
...
```{python relpace-column,warning=FALSE,results="asis"}
...
```{python drop-column,results="asis"}
...
```{python pop-column,results="asis"}
sd.pop("newcol3")
sd.pipe(lambda df:print(df.to_markdown(tablefmt="github")))
```
...
```{python add-append,results="asis"}
pd.concat(
...
```{python replace-row,results="asis"}
sd[4:5] = sd[3:4]
sd.pipe(lambda df:print(df.to_markdown(tablefmt="github")))
```
...
```{python index-column,results="asis"}
...
```{r numpy-function,echo=FALSE}
knitr::include_graphics("static/image/数据/算数运算.png")
```

```{r numpy-function-add,echo=FALSE}
knitr::include_graphics("static/image/数据/数学函数补充.png")
```

```{python numpy-function-apply}
np.floor(sd.math)
np.round(sd[["math"]])
```
...
```{python series-scalar-math}
sd.math + 3
```

#### 逻辑运算

```{python series-scalar-logical}
sd.math > 80
...
```{python dataframe-math}
sd[["math","english"]] + sd[["math","english"]]
```

#### 逻辑运算

```{python dataframe-logical}
sd[["math","english"]] > 80
...
```{r numpy-dataframe-function,echo=FALSE}
knitr::include_graphics("static/image/数据/二元通用函数.png")
```
...
```{r statiscal-method,echo=FALSE}
knitr::include_graphics("static/image/数据/描述性统计.png")
```

对于数据框来说，它的sum，min等聚合函数可以选择axis，甚至可以选择index来聚合,但是不能再指定level了。

```{python statiscal-method-code}
multi_index.sum(axis=0)
```

```{r statiscal-method-add,echo=FALSE}
knitr::include_graphics("static/image/数据/统计方法.png")
```
